...
class AnnounceChatConfirmationAction extends Action2 {
	constructor() {
		super({
			...
			category: { value: localize('hify.chat.category', 'Chat'), original: 'Chat' },
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (isResponseVM(lastResponse)) {
			const confirmationWidgets = lastFocusedWidget.domNode.querySelectorAll('.hify-chat-confirmation-widget-container');
