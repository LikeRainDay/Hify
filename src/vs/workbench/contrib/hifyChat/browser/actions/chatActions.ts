...
export const CHAT_CATEGORY = localize2('hify.chat.category', 'Chat');
	...
	registerAction2(class ChatHistoryAction extends Action2 {
		constructor() {
			super({
				...
				title: localize2('hify.chat.history.label', "Show Chats..."),
				menu: [
					{
						id: MenuId.ViewTitle,
		...
		private showLegacyPicker = async (
			...
			const renameButton: IQuickInputButton = {
				...
				tooltip: localize('hify.chat.history.rename', "Rename"),
			};
		...
		private showIntegratedPicker = async (
			...
			const getPicks = async (showAllChats: boolean = false, showAllAgents: boolean = false) => {
				...
				if (fastPickItems.length > 0) {
					fastPicks.push({
						type: 'separator',
						label: localize('hify.chat.history.recent', 'Recent Chats'),
					});
					...
					if (!showAllChats && allFastPickItems.length > 5) {
						fastPicks.push({
							label: localize('hify.chat.history.showMore', 'Show more...'),
							description: '',
							chat: {
						...
						try {
							...
							if (agentPicks.length > 0) {
								...
								if (!showAllAgents && providerNSessions.length > 5) {
									currentPicks.push({
										label: localize('hify.chat.history.showMoreAgents', 'Show more...'),
										description: '',
										chat: {
	...
	registerAction2(class OpenChatInNewEditorGroupAction extends Action2 {
		constructor() {
			super({
				...
				title: localize2('hify.chat.openNewChatToTheSide.label', "Open New Chat Editor to the Side"),
				category: CHAT_CATEGORY,
				precondition: ChatContextKeys.enabled,
				f1: false,
				menu: {
	...
	registerAction2(class ClearChatHistoryAction extends Action2 {
		constructor() {
			super({
				...
				title: localize2('hify.chat.clear.label', "Clear All Workspace Chats"),
				precondition: ChatContextKeys.enabled,
				category: CHAT_CATEGORY,
				f1: true,
			});
				...
				buttons: [
					{
						...
						run: () => {
							...
							telemetryService.publicLog2<WorkbenchActionExecutedEvent, WorkbenchActionExecutedClassification>('workbenchActionExecuted', { id: commandId, from: 'hify-chat-dialog' });
...
// TODO@bpasero remove these when Chat extension is built-in
{
	...
	registerGenerateCodeCommand('hify.chat.internal.explain', 'github.copilot.chat.explain');
	registerGenerateCodeCommand('hify.chat.internal.fix', 'github.copilot.chat.fix');
	registerGenerateCodeCommand('hify.chat.internal.review', 'github.copilot.chat.review');
	registerGenerateCodeCommand('hify.chat.internal.generateDocs', 'github.copilot.chat.generateDocs');
	registerGenerateCodeCommand('hify.chat.internal.generateTests', 'github.copilot.chat.generateTests');
	...
	MenuRegistry.appendMenuItem(MenuId.ChatTextEditorMenu, {
		command: {
			id: 'hify.chat.internal.generateDocs',
	...
	MenuRegistry.appendMenuItem(MenuId.ChatTextEditorMenu, {
		command: {
			id: 'hify.chat.internal.generateTests',
...
registerAction2(class ToggleDefaultVisibilityAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('hify.chat.toggleDefaultVisibility.label', "Show View by Default"),
...
registerAction2(class EditToolApproval extends Action2 {
	constructor() {
		super({
			...
			title: localize2('hify.chat.editToolApproval.label', "Edit Tool Approval"),
			f1: false
		});
	}
	...
	async run(accessor: ServicesAccessor, toolId: string): Promise<void> {
		...
		const items: TItem[] = [
			{ id: 'never', label: localize('hify.chat.toolApproval.manual', "Always require manual approval") },
			{ id: 'session', label: localize('hify.chat.toolApproval.session', "Auto-approve for this session") },
			{ id: 'workspace', label: localize('hify.chat.toolApproval.workspace', "Auto-approve for this workspace") },
			{ id: 'profile', label: localize('hify.chat.toolApproval.profile', "Auto-approve globally") }
		];

		const quickPick = quickInputService.createQuickPick<TItem>();
		quickPick.placeholder = localize('hify.chat.editToolApproval.title', "Approval setting for {0}", tool.displayName ?? tool.id);
