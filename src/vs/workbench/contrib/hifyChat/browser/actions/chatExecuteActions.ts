		...
		if (widget?.viewModel?.editing) {
			...
			if (requestId) {
				...
				if (!confirmation.confirmed) {
					telemetryService.publicLog2<EditUndoEvent, EditUndoEventClassification>('hify.chat.undoEditsConfirmation', {
						editRequestType: configurationService.getValue<string>('hify.chat.editRequests'),
						outcome: 'cancelled',
...
class ToggleChatModeAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: any[]) {
		...
		telemetryService.publicLog2<ChatModeChangeEvent, ChatModeChangeClassification>('hify.chat.modeChange', {
...
class SendToNewChatAction extends Action2 {
	constructor() {
		...
		super({
			...
			title: localize2('hify.chat.newChat.label', "Send to New Chat"),
			precondition,
			category: CHAT_CATEGORY,
			f1: false,
			menu: {
