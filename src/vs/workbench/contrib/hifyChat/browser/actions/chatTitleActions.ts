	...
	registerAction2(class RetryChatAction extends Action2 {
		constructor() {
			super({
				...
				title: localize2('hify.chat.retry.label', "Retry"),
				f1: false,
				category: CHAT_CATEGORY,
		...
		async run(accessor: ServicesAccessor, ...args: any[]) {
			...
			if (chatModel && (mode === ChatModeKind.Edit || mode === ChatModeKind.Agent)) {
				...
				const shouldPrompt = entriesModifiedInLastRequest.length > 0 && configurationService.getValue('hify.chat.editing.confirmEditRequestRetry') === true;
				const confirmation = shouldPrompt
					? await dialogService.confirm({
						title: localize('hify.chat.retryLast.confirmation.title2', "Do you want to retry your last request?"),
						message: entriesModifiedInLastRequest.length === 1
							? localize('hify.chat.retry.confirmation.message2', "This will undo edits made to {0} since this request.", basename(entriesModifiedInLastRequest[0].modifiedURI))
							: localize('hify.chat.retryLast.confirmation.message2', "This will undo edits made to {0} files in your working set since this request. Do you want to proceed?", entriesModifiedInLastRequest.length),
						primaryButton: localize('hify.chat.retry.confirmation.primaryButton', "Yes"),
						checkbox: { label: localize('hify.chat.retry.confirmation.checkbox', "Don't ask again"), checked: false },
						type: 'info'
					})
					: { confirmed: true };
				...
				if (confirmation.checkboxChecked) {
					await configurationService.updateValue('hify.chat.editing.confirmEditRequestRetry', false);
				}
