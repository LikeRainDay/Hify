...
class AcceptToolConfirmation extends Action2 {
	constructor() {
		super({
			...
			title: localize2('hify.chat.accept', "Accept"),
			f1: false,
	...
	override async run(accessor: ServicesAccessor, ...args: any[]): Promise<void> {
		...
		switch (entriesScope) {
			case ToolsScope.Session:
				placeholder = localize('hify.chat.tools.placeholder.session', "Select tools for this chat session");
				description = localize('hify.chat.tools.description.session', "The selected tools were configured by a prompt command and only apply to this chat session.");
				break;
			case ToolsScope.Mode:
				placeholder = localize('hify.chat.tools.placeholder.mode', "Select tools for this chat mode");
				description = localize('hify.chat.tools.description.mode', "The selected tools are configured by the '{0}' chat mode. Changes to the tools will be applied to the mode file as well.", widget.input.currentModeObs.get().label);
				break;
			case ToolsScope.Global:
				placeholder = localize('hify.chat.tools.placeholder.global', "Select tools that are available to chat.");
...
class ConfigureToolsActionRendering implements IWorkbenchContribution {
	...
	static readonly ID = 'hify.chat.configureToolsActionRendering';
