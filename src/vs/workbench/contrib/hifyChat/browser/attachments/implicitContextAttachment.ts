...
export class ImplicitContextAttachmentWidget extends Disposable {
	...
	constructor(
		...
		this.domNode = dom.$('.hify-chat-attached-context-attachment.show-file-icons.implicit');
		this.render();
	}
	...
	private render() {
		...
		const ariaLabel = range ? localize('hify.chat.fileAttachmentWithRange', "Attached {0}, {1}, line {2} to line {3}", attachmentTypeName, friendlyName, range.startLineNumber, range.endLineNumber) : localize('hify.chat.fileAttachment', "Attached {0}, {1}", attachmentTypeName, friendlyName);
		...
		const isSuggestedEnabled = this.configService.getValue('hify.chat.implicitContext.suggestedContext');
