...
configurationRegistry.registerConfiguration({
	...
	properties: {
		'hify.chat.fontSize': {
			type: 'number',
			description: nls.localize('hify.chat.fontSize', "Controls the font size in pixels in chat messages."),
			default: 13,
			minimum: 6,
			maximum: 100
		},
		'hify.chat.fontFamily': {
			type: 'string',
			description: nls.localize('hify.chat.fontFamily', "Controls the font family in chat messages."),
			default: 'default'
		},
		'hify.chat.editor.fontSize': {
			type: 'number',
			description: nls.localize('interactiveSession.editor.fontSize', "Controls the font size in pixels in chat codeblocks."),
			default: isMacintosh ? 12 : 14,
		},
		'hify.chat.editor.fontFamily': {
			type: 'string',
			description: nls.localize('interactiveSession.editor.fontFamily', "Controls the font family in chat codeblocks."),
			default: 'default'
		},
		'hify.chat.editor.fontWeight': {
			type: 'string',
			description: nls.localize('interactiveSession.editor.fontWeight', "Controls the font weight in chat codeblocks."),
			default: 'default'
		},
		'hify.chat.editor.wordWrap': {
			type: 'string',
			description: nls.localize('interactiveSession.editor.wordWrap', "Controls whether lines should wrap in chat codeblocks."),
			default: 'off',
			enum: ['on', 'off']
		},
		'hify.chat.editor.lineHeight': {
			type: 'number',
			description: nls.localize('interactiveSession.editor.lineHeight', "Controls the line height in pixels in chat codeblocks. Use 0 to compute the line height from the font size."),
			default: 0
		},
		'hify.chat.commandCenter.enabled': {
			type: 'boolean',
			markdownDescription: nls.localize('hify.chat.commandCenter.enabled', "Controls whether the command center shows a menu for actions to control chat (requires {0}).", '`#window.commandCenter#`'),
			default: true
		},
		'hify.chat.implicitContext.enabled': {
			type: 'object',
			tags: ['experimental'],
			description: nls.localize('hify.chat.implicitContext.enabled.1', "Enables automatically using the active editor as chat context for specified chat locations."),
			additionalProperties: {
				type: 'string',
				enum: ['never', 'first', 'always'],
				description: nls.localize('hify.chat.implicitContext.value', "The value for the implicit context."),
				enumDescriptions: [
					nls.localize('hify.chat.implicitContext.value.never', "Implicit context is never enabled."),
					nls.localize('hify.chat.implicitContext.value.first', "Implicit context is enabled for the first interaction."),
					nls.localize('hify.chat.implicitContext.value.always', "Implicit context is always enabled.")
				]
			},
			default: {
				'panel': 'always',
		...
		'hify.chat.implicitContext.suggestedContext': {
			type: 'boolean',
			tags: ['experimental'],
			markdownDescription: nls.localize('hify.chat.implicitContext.suggestedContext', "Controls whether the new implicit context flow is shown. In Ask and Edit modes, the context will automatically be included. In Agent mode context will be suggested as an attachment. Selections are always included as context."),
			default: true,
		},
		'hify.chat.editing.autoAcceptDelay': {
			type: 'number',
			markdownDescription: nls.localize('hify.chat.editing.autoAcceptDelay', "Delay after which changes made by chat are automatically accepted. Values are in seconds, `0` means disabled and `100` seconds is the maximum."),
			default: 0,
			minimum: 0,
			maximum: 100
		},
		'hify.chat.editing.confirmEditRequestRemoval': {
			type: 'boolean',
			scope: ConfigurationScope.APPLICATION,
			markdownDescription: nls.localize('hify.chat.editing.confirmEditRequestRemoval', "Whether to show a confirmation before removing a request and its associated edits."),
			default: true,
		},
		'hify.chat.editing.confirmEditRequestRetry': {
			type: 'boolean',
			scope: ConfigurationScope.APPLICATION,
			markdownDescription: nls.localize('hify.chat.editing.confirmEditRequestRetry', "Whether to show a confirmation before retrying a request and its associated edits."),
			default: true,
		},
		'hify.chat.experimental.detectParticipant.enabled': {
			type: 'boolean',
			deprecationMessage: nls.localize('hify.chat.experimental.detectParticipant.enabled.deprecated', "This setting is deprecated. Please use `chat.detectParticipant.enabled` instead."),
			description: nls.localize('hify.chat.experimental.detectParticipant.enabled', "Enables chat participant autodetection for panel chat."),
			default: null
		},
		'hify.chat.detectParticipant.enabled': {
			type: 'boolean',
			description: nls.localize('hify.chat.detectParticipant.enabled', "Enables chat participant autodetection for panel chat."),
			default: true
		},
		'hify.chat.renderRelatedFiles': {
			type: 'boolean',
			description: nls.localize('hify.chat.renderRelatedFiles', "Controls whether related files should be rendered in the chat input."),
			default: false
		},
		'hify.chat.notifyWindowOnConfirmation': {
			type: 'boolean',
			description: nls.localize('hify.chat.notifyWindowOnConfirmation', "Controls whether a chat session should notify the user when a confirmation is needed while the window is not in focus. This includes a window badge as well as notification toast."),
			default: true,
		},
		...
		[ChatConfiguration.AutoApproveEdits]: {
			...
			markdownDescription: nls.localize('hify.chat.tools.autoApprove.edits', "Controls whether edits made by chat are automatically approved. The default is to approve all edits except those made to certain files which have the potential to cause immediate unintended side-effects, such as `**/.vscode/*.json`.\n\nFiles are matched against the glob patterns in the order they are specified."),
			type: 'object',
			additionalProperties: {
				type: 'boolean',
			}
		},
		'hify.chat.sendElementsToChat.enabled': {
			default: true,
			description: nls.localize('hify.chat.sendElementsToChat.enabled', "Controls whether elements can be sent to chat from the Simple Browser."),
			type: 'boolean',
			tags: ['experimental']
		},
		'hify.chat.sendElementsToChat.attachCSS': {
			default: true,
			markdownDescription: nls.localize('hify.chat.sendElementsToChat.attachCSS', "Controls whether CSS of the selected element will be added to the chat. {0} must be enabled.", '`#chat.sendElementsToChat.enabled#`'),
			type: 'boolean',
			tags: ['experimental']
		},
		'hify.chat.sendElementsToChat.attachImages': {
			default: true,
			markdownDescription: nls.localize('hify.chat.sendElementsToChat.attachImages', "Controls whether a screenshot of the selected element will be added to the chat. {0} must be enabled.", '`#chat.sendElementsToChat.enabled#`'),
			type: 'boolean',
			tags: ['experimental']
		},
		'hify.chat.undoRequests.restoreInput': {
			default: true,
			markdownDescription: nls.localize('hify.chat.undoRequests.restoreInput', "Controls whether the input of the chat should be restored when an undo request is made. The input will be filled with the text of the request that was restored."),
			type: 'boolean',
			tags: ['experimental']
		},
		'hify.chat.editRequests': {
			markdownDescription: nls.localize('hify.chat.editRequests', "Enables editing of requests in the chat. This allows you to change the request content and resubmit it to the model."),
			type: 'string',
			enum: ['inline', 'hover', 'input', 'none'],
			default: 'inline',
		},
		'hify.chat.emptyChatState.enabled': {
			type: 'boolean',
			default: true,
			description: nls.localize('hify.chat.emptyChatState', "Shows a modified empty chat state with hints in the input placeholder text."),
			tags: ['experimental'],
			experiment: {
				mode: 'startup'
			}
		...
		[ChatConfiguration.EmptyStateHistoryEnabled]: {
			type: 'boolean',
			default: false,
			description: nls.localize('hify.chat.emptyState.history.enabled', "Show recent chat history on the empty chat state."),
			tags: ['experimental']
		},
		'hify.chat.checkpoints.enabled': {
			type: 'boolean',
			default: true,
			description: nls.localize('hify.chat.checkpoints.enabled', "Enables checkpoints in chat. Checkpoints allow you to restore the chat to a previous state."),
		},
		'hify.chat.checkpoints.showFileChanges': {
			type: 'boolean',
			description: nls.localize('hify.chat.checkpoints.showFileChanges', "Controls whether to show chat checkpoint file changes."),
			default: false
		},
		[mcpAccessConfig]: {
			type: 'string',
			description: nls.localize('hify.chat.mcp.access', "Controls access to installed Model Context Protocol servers."),
			enum: [
				McpAccessValue.None,
			...
			enumDescriptions: [
				nls.localize('hify.chat.mcp.access.none', "No access to MCP servers."),
				nls.localize('hify.chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that VS Code is connected to."),
				nls.localize('hify.chat.mcp.access.any', "Allow access to any installed MCP server.")
			],
		...
		[mcpAutoStartConfig]: {
			type: 'string',
			description: nls.localize('hify.chat.mcp.autostart', "Controls whether MCP servers should be automatically started when the chat messages are submitted."),
			...
			enumDescriptions: [
				nls.localize('hify.chat.mcp.autostart.never', "Never automatically start MCP servers."),
				nls.localize('hify.chat.mcp.autostart.onlyNew', "Only automatically start new MCP servers that have never been run."),
				nls.localize('hify.chat.mcp.autostart.newAndOutdated', "Automatically start new and outdated MCP servers that are not yet running.")
			],
			tags: ['experimental'],
		},
		[mcpServerSamplingSection]: {
			type: 'object',
			description: nls.localize('hify.chat.mcp.serverSampling', "Configures which models are exposed to MCP servers for sampling (making model requests in the background). This setting can be edited in a graphical way under the `{0}` command.", 'MCP: ' + nls.localize('mcp.list', 'List Servers')),
			...
			additionalProperties: {
				...
				properties: {
					allowedDuringChat: {
						type: 'boolean',
						description: nls.localize('hify.chat.mcp.serverSampling.allowedDuringChat', "Whether this server is make sampling requests during its tool calls in a chat session."),
						default: true,
					},
					allowedOutsideChat: {
						type: 'boolean',
						description: nls.localize('hify.chat.mcp.serverSampling.allowedOutsideChat', "Whether this server is allowed to make sampling requests outside of a chat session."),
						default: false,
					},
					allowedModels: {
						type: 'array',
						items: {
							type: 'string',
							description: nls.localize('hify.chat.mcp.serverSampling.model', "A model the MCP server has access to."),
						},
					}
				}
			},
		...
		[AssistedTypes[AddConfigurationType.NuGetPackage].enabledConfigKey]: {
			type: 'boolean',
			description: nls.localize('hify.chat.mcp.assisted.nuget.enabled.description', "Enables NuGet packages for AI-assisted MCP server installation. Used to install MCP servers by name from the central registry for .NET packages (NuGet.org)."),
			default: false,
			tags: ['experimental'],
			experiment: {
				mode: 'startup'
		...
		[ChatConfiguration.Edits2Enabled]: {
			type: 'boolean',
			description: nls.localize('hify.chat.edits2Enabled', "Enable the new Edits mode that is based on tool-calling. When this is enabled, models that don't support tool-calling are unavailable for Edits mode."),
			default: false,
		},
		[ChatConfiguration.ExtensionToolsEnabled]: {
			type: 'boolean',
			description: nls.localize('hify.chat.extensionToolsEnabled', "Enable using tools contributed by third-party extensions."),
			default: true,
			policy: {
				name: 'ChatAgentExtensionTools',
				minimumVersion: '1.99',
				description: nls.localize('hify.chat.extensionToolsPolicy', "Enable using tools contributed by third-party extensions."),
			}
		},
		[ChatConfiguration.AgentEnabled]: {
			type: 'boolean',
			description: nls.localize('hify.chat.agent.enabled.description', "Enable agent mode for chat. When this is enabled, agent mode can be activated via the dropdown in the view."),
			default: true,
			policy: {
				name: 'ChatAgentMode',
				minimumVersion: '1.99',
		...
		[ChatConfiguration.EnableMath]: {
			type: 'boolean',
			description: nls.localize('hify.chat.mathEnabled.description', "Enable math rendering in chat responses using KaTeX."),
			default: true,
			tags: ['preview'],
		},
		[ChatConfiguration.AgentSessionsViewLocation]: {
			type: 'string',
			enum: ['disabled', 'view'],
			description: nls.localize('hify.chat.sessionsViewLocation.description', "Controls where to show the agent sessions menu."),
			default: 'disabled',
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
		...
		[PromptsConfig.KEY]: {
			type: 'boolean',
			title: nls.localize(
				'hify.chat.reusablePrompts.config.enabled.title',
				"Prompt Files",
			),
			markdownDescription: nls.localize(
				'hify.chat.reusablePrompts.config.enabled.description',
			...
			policy: {
				...
				minimumVersion: '1.99',
				description: nls.localize('hify.chat.promptFiles.policy', "Enables reusable prompt and instruction files in Chat sessions.")
			}
		},
		[PromptsConfig.INSTRUCTIONS_LOCATION_KEY]: {
			type: 'object',
			title: nls.localize(
				'hify.chat.instructions.config.locations.title',
				"Instructions File Locations",
			),
			markdownDescription: nls.localize(
				'hify.chat.instructions.config.locations.description',
		...
		[PromptsConfig.PROMPT_LOCATIONS_KEY]: {
			type: 'object',
			title: nls.localize(
				'hify.chat.reusablePrompts.config.locations.title',
				"Prompt File Locations",
			),
			markdownDescription: nls.localize(
				'hify.chat.reusablePrompts.config.locations.description',
		...
		[PromptsConfig.MODE_LOCATION_KEY]: {
			type: 'object',
			title: nls.localize(
				'hify.chat.mode.config.locations.title',
				"Mode File Locations",
			),
			markdownDescription: nls.localize(
				'hify.chat.mode.config.locations.description',
		...
		[PromptsConfig.USE_AGENT_MD]: {
			type: 'boolean',
			title: nls.localize('hify.chat.useAgentMd.title', "Use AGENTS.MD file",),
			markdownDescription: nls.localize('hify.chat.useAgentMd.description', "Controls whether instructions from `AGENTS.MD` file found in a workspace roots are added to all chat requests.",),
			default: true,
			restricted: true,
			disallowConfigurationDefault: true,
			tags: ['experimental', 'prompts', 'reusable prompts', 'prompt snippets', 'instructions']
		...
		[PromptsConfig.PROMPT_FILES_SUGGEST_KEY]: {
			type: 'object',
			title: nls.localize(
				'hify.chat.promptFilesRecommendations.title',
				"Prompt File Recommendations",
			),
			markdownDescription: nls.localize(
				'hify.chat.promptFilesRecommendations.description',
		...
		'hify.chat.setup.signInDialogVariant': { // TODO@bpasero remove me eventually
			type: 'string',
			enum: ['default', 'apple'],
			description: nls.localize('hify.chat.signInDialogVariant', "Control variations of the sign-in dialog."),
			default: 'default',
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
		...
		'hify.chat.todoListWidget.position': {
			type: 'string',
			default: 'default',
			enum: ['default', 'off', 'hify-chat-input'],
			enumDescriptions: [
				nls.localize('hify.chat.todoListWidget.position.default', "Show todo list widget in the default position at the top of the chat panel."),
				nls.localize('hify.chat.todoListWidget.position.off', "Hide the todo list widget."),
				nls.localize('hify.chat.todoListWidget.position.chatInput', "Show todo list widget near the chat input.")
			],
			description: nls.localize('hify.chat.todoListWidget.position', "Controls the position of the todo list widget in chat."),
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
			}
		},
		'hify.chat.todoListTool.writeOnly': {
			type: 'boolean',
			default: false,
			description: nls.localize('hify.chat.todoListTool.writeOnly', "When enabled, the todo tool operates in write-only mode, requiring the agent to remember todos in context."),
			tags: ['experimental']
		},
		[ChatConfiguration.ThinkingStyle]: {
			...
			enumDescriptions: [
				nls.localize('hify.chat.agent.thinkingMode.collapsed', "Thinking parts will be collapsed by default."),
				nls.localize('hify.chat.agent.thinkingMode.collapsedPreview', "Thinking parts will be expanded first, then collapse once we reach a part that is not thinking."),
				nls.localize('hify.chat.agent.thinkingMode.expanded', "Thinking parts will be expanded by default."),
				nls.localize('hify.chat.agent.thinkingMode.none', "Do not show the thinking"),
				nls.localize('hify.chat.agent.thinkingMode.collapsedPerItem', "Each thinking subsection is individually collapsible and collapsed by default inside the thinking block."),
			],
			description: nls.localize('hify.chat.agent.thinkingStyle', "Controls how thinking is rendered."),
			tags: ['experimental'],
		},
		'hify.chat.disableAIFeatures': {
			type: 'boolean',
			description: nls.localize('hify.chat.disableAIFeatures', "Disable and hide built-in AI features provided by GitHub Copilot, including chat, code completions and next edit suggestions."),
			default: false,
			scope: ConfigurationScope.WINDOW
		},
		[ChatConfiguration.UseChatSessionsForCloudButton]: {
			type: 'boolean',
			description: nls.localize('hify.chat.useChatSessionsForCloudButton', "Controls whether the 'Delegate to coding agent' button uses the new chat sessions API."),
			default: false,
			tags: ['experimental'],
		...
		[ChatConfiguration.ShowAgentSessionsViewDescription]: {
			type: 'boolean',
			description: nls.localize('hify.chat.showAgentSessionsViewDescription', "Controls whether session descriptions are displayed on a second row in the Chat Sessions view."),
			default: true,
		},
		'hify.chat.allowAnonymousAccess': { // TODO@bpasero remove me eventually
			type: 'boolean',
			description: nls.localize('hify.chat.allowAnonymousAccess', "Controls whether anonymous access is allowed in chat."),
			default: false,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
...
Registry.as<IConfigurationMigrationRegistry>(Extensions.ConfigurationMigration).registerConfigurationMigrations([
	{
		key: 'hify.chat.experimental.detectParticipant.enabled',
		migrateFn: (value, _accessor) => ([
			['hify.chat.experimental.detectParticipant.enabled', { value: undefined }],
			['hify.chat.detectParticipant.enabled', { value: value !== false }]
		])
	},
	{
		...
		const registerMaxRequestsSetting = () => {
			...
			this.experimentService.getTreatment<number>(treatmentId).then(value => {
				...
				const node: IConfigurationNode = {
					...
					type: 'object',
					properties: {
						'hify.chat.agent.maxRequests': {
							type: 'number',
							markdownDescription: nls.localize('hify.chat.agent.maxRequests', "The maximum number of requests to allow per-turn in agent mode. When the limit is reached, will ask to confirm to continue."),
...
class ChatSlashStaticSlashCommandsContribution extends Disposable {
	...
	constructor(
		...
		this._store.add(slashCommandService.registerSlashCommand({
			...
			detail: nls.localize('save-hify-chat-to-prompt-file', "Save chat to a prompt file"),
