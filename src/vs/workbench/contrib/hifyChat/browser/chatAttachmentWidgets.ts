...
abstract class AbstractChatAttachmentWidget extends Disposable {
	...
	constructor(
		...
		super();
		this.element = dom.append(container, $('.hify-chat-attached-context-attachment.show-file-icons'));
	...
	protected attachClearButton() {
		...
		const clearButton = new Button(this.element, {
			...
			title: localize('hify.chat.attachment.clearButton', "Remove from context")
		});
...
export class FileAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
		...
		let ariaLabel = range ? localize('hify.chat.fileAttachmentWithRange', "Attached file, {0}, line {1} to line {2}", friendlyName, range.startLineNumber, range.endLineNumber) : localize('hify.chat.fileAttachment', "Attached file, {0}", friendlyName);

		if (attachment.omittedState === OmittedState.Full) {
			ariaLabel = localize('hify.chat.omittedFileAttachment', "Omitted this file: {0}", attachment.name);
	...
	private renderOmittedWarning(friendlyName: string, ariaLabel: string, hoverDelegate: IHoverDelegate) {
		const pillIcon = dom.$('div.hify-chat-attached-context-pill', {}, dom.$('span.codicon.codicon-warning'));
		const textLabel = dom.$('span.hify-chat-attached-context-custom-text', {}, friendlyName);
		this.element.appendChild(pillIcon);
		this.element.appendChild(textLabel);

		const hoverElement = dom.$('div.hify-chat-attached-context-hover');
		hoverElement.setAttribute('aria-label', ariaLabel);
		this.element.classList.add('warning');

		hoverElement.textContent = localize('hify.chat.fileAttachmentHover', "{0} does not support this file type.", this.currentLanguageModel ? this.languageModelsService.lookupLanguageModel(this.currentLanguageModel.identifier)?.name : this.currentLanguageModel ?? 'This model');
...
export class ImageAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
		...
		if (attachment.omittedState === OmittedState.Full) {
			ariaLabel = localize('hify.chat.omittedImageAttachment', "Omitted this image: {0}", attachment.name);
		} else if (attachment.omittedState === OmittedState.Partial) {
			ariaLabel = localize('hify.chat.partiallyOmittedImageAttachment', "Partially omitted this image: {0}", attachment.name);
		} else {
			ariaLabel = localize('hify.chat.imageAttachment', "Attached image, {0}", attachment.name);
		}
...
function createImageElements(resource: URI | undefined, name: string, fullName: string,
	...
	const pillIcon = dom.$('div.hify-chat-attached-context-pill', {}, dom.$(supportsVision ? 'span.codicon.codicon-file-media' : 'span.codicon.codicon-warning'));
	const textLabel = dom.$('span.hify-chat-attached-context-custom-text', {}, name);
	...
	if ((!supportsVision && currentLanguageModel) || omittedState === OmittedState.Full) {
		...
		hoverElement.textContent = localize('hify.chat.imageAttachmentHover', "{0} does not support images.", currentLanguageModelName ?? 'This model');
		...
		const pillImg = dom.$('img.hify-chat-attached-context-pill-image', { src: url, alt: '' });
		const pill = dom.$('div.hify-chat-attached-context-pill', {}, pillImg);

		const existingPill = element.querySelector('.hify-chat-attached-context-pill');
		if (existingPill) {
			existingPill.replaceWith(pill);
		}

		const hoverImage = dom.$('img.hify-chat-attached-context-image', { src: url, alt: '' });
		const imageContainer = dom.$('div.hify-chat-attached-context-image-container', {}, hoverImage);
		hoverElement.appendChild(imageContainer);

		if (resource) {
			const urlContainer = dom.$('a.hify-chat-attached-context-url', {}, omittedState === OmittedState.Partial ? localize('hify.chat.imageAttachmentWarning', "This GIF was partially omitted - current frame will be sent.") : fullName);
			const separator = dom.$('div.hify-chat-attached-context-url-separator');
		...
		hoverImage.onerror = () => {
			...
			const pillIcon = dom.$('div.hify-chat-attached-context-pill', {}, dom.$('span.codicon.codicon-file-media'));
			const pill = dom.$('div.hify-chat-attached-context-pill', {}, pillIcon);
			const existingPill = element.querySelector('.hify-chat-attached-context-pill');
			if (existingPill) {
				existingPill.replaceWith(pill);
			}
...
export class PasteAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
		...
		const ariaLabel = localize('hify.chat.attachment', "Attached context, {0}", attachment.name);
		this.element.ariaLabel = ariaLabel;
...
export class DefaultChatAttachmentWidget extends AbstractChatAttachmentWidget {
	constructor(
		...
		this.element.ariaLabel = localize('hify.chat.attachment', "Attached context, {0}", attachment.name);
...
export class PromptFileAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	private updateLabel(attachment: IPromptFileVariableEntry) {
		...
		const ariaLabel = isPrompt
			? localize('hify.chat.promptAttachment', "Prompt file, {0}", friendlyName)
			: localize('hify.chat.instructionsAttachment', "Instructions attachment, {0}", friendlyName);
...
export class ToolSetOrToolItemAttachmentWidget extends AbstractChatAttachmentWidget {
	constructor(
		...
		this.element.style.cursor = 'pointer';
		this.element.ariaLabel = localize('hify.chat.attachment', "Attached context, {0}", name);
...
export class NotebookCellOutputChatAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	getAriaLabel(attachment: INotebookOutputVariableEntry): string {
		return localize('hify.chat.NotebookImageAttachment', "Attached Notebook output, {0}", attachment.name);
	}
	...
	private renderImageOutput(resource: URI, attachment: INotebookOutputVariableEntry) {
		let ariaLabel: string;
		if (attachment.omittedState === OmittedState.Full) {
			ariaLabel = localize('hify.chat.omittedNotebookImageAttachment', "Omitted this Notebook ouput: {0}", attachment.name);
		} else if (attachment.omittedState === OmittedState.Partial) {
			ariaLabel = localize('hify.chat.partiallyOmittedNotebookImageAttachment', "Partially omitted this Notebook output: {0}", attachment.name);
		} else {
...
export class ElementChatAttachmentWidget extends AbstractChatAttachmentWidget {
	constructor(
		...
		const ariaLabel = localize('hify.chat.elementAttachment', "Attached element, {0}", attachment.name);
		this.element.ariaLabel = ariaLabel;
		...
		const attachmentLabel = attachment.name;
		const withIcon = attachment.icon?.id ? `$(${attachment.icon.id})\u00A0${attachmentLabel}` : attachmentLabel;
		this.label.setLabel(withIcon, undefined, { title: localize('hify.chat.clickToViewContents', "Click to view the contents of: {0}", attachmentLabel) });
