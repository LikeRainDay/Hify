...
export class ChatQueryTitlePart extends Disposable {
	...
	constructor(
		...
		element.classList.add('hify-chat-query-title-part');
...
abstract class BaseSimpleChatConfirmationWidget<T> extends Disposable {
	...
	constructor(
		...
		const elements = dom.h('.hify-chat-confirmation-widget-container@container', [
			dom.h('.hify-chat-confirmation-widget@root', [
				dom.h('.hify-chat-confirmation-widget-title@title'),
				dom.h('.hify-chat-confirmation-widget-message@message'),
				dom.h('.hify-chat-buttons-container@buttonsContainer', [
					dom.h('.hify-chat-buttons@buttons'),
					dom.h('.hify-chat-toolbar@toolbar'),
				]),
			]),
		]);
	...
	protected renderMessage(element: HTMLElement, listContainer: HTMLElement): void {
		...
		if (this.showingButtons && this._configurationService.getValue<boolean>('hify.chat.notifyWindowOnConfirmation')) {
...
abstract class BaseChatConfirmationWidget<T> extends Disposable {
	...
	constructor(
		...
		const elements = dom.h('.hify-chat-confirmation-widget-container@container', [
			dom.h('.hify-chat-confirmation-widget2@root', [
				dom.h('.hify-chat-confirmation-widget-title', [
					dom.h('.hify-chat-title@title'),
					dom.h('.hify-chat-toolbar-container@buttonsContainer', [
						dom.h('.hify-chat-toolbar@toolbar'),
					]),
				]),
				dom.h('.hify-chat-confirmation-widget-message@message'),
				dom.h('.hify-chat-confirmation-widget-buttons', [
					dom.h('.hify-chat-buttons@buttons'),
				]),
			]),]);
...
function configureAccessibilityContainer(container: HTMLElement, title: string | IMarkdownString, message?: string | IMarkdownString | HTMLElement): void {
	...
	container.setAttribute('aria-label', localize('hify.chat.confirmationWidget.ariaLabel', "Chat Confirmation Dialog {0} {1}", titleAsString, messageAsString));
	container.classList.add('hify-chat-confirmation-widget-container');
}
