...
export class ChatMarkdownContentPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		this.domNode = $('div.hify-chat-markdown-part');
...
export class CollapsedCodeBlock extends Disposable {
	...
	constructor(
		...
		super();
		this.element = $('.hify-chat-codeblock-pill-widget');
	...
	render(uri: URI, isStreaming?: boolean): void {
		...
		if (isStreaming) {
			labelDetail.textContent = localize('hify.chat.codeblock.generating', "Generating edits...");
		}
		...
		const renderDiff = (changes: IEditSessionEntryDiff | undefined) => {
			...
			if (changes && !changes?.identical && !changes?.quitEarly) {
				...
				const insertionsFragment = changes.added === 1 ? localize('hify.chat.codeblock.insertions.one', "1 insertion") : localize('hify.chat.codeblock.insertions', "{0} insertions", changes.added);
				const deletionsFragment = changes.removed === 1 ? localize('hify.chat.codeblock.deletions.one', "1 deletion") : localize('hify.chat.codeblock.deletions', "{0} deletions", changes.removed);
		...
		this._progressStore.add(autorun(r => {
			...
			if (!isStreaming && !isComplete) {
				...
				labelDetail.textContent = value === 0 || !value ? localize('hify.chat.codeblock.generating', "Generating edits...") : localize('hify.chat.codeblock.applyingPercentage', "Applying edits ({0}%)...", Math.round(value * 100));
