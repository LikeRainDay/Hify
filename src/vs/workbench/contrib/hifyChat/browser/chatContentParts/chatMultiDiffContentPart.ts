...
export class ChatMultiDiffContentPart extends Disposable implements IChatContentPart {
	...
	private renderHeader(container: HTMLElement): IDisposable {
		...
		const viewListButtonContainer = container.appendChild($('.hify-chat-file-changes-label'));
		...
		const setExpansionState = () => {
			...
			this.domNode.classList.toggle('hify-chat-file-changes-collapsed', this.isCollapsed);
	...
	private renderViewAllFileChangesButton(container: HTMLElement): IDisposable {
		const button = container.appendChild($('.hify-chat-view-changes-icon'));
	...
	private renderContributedButtons(container: HTMLElement): IDisposable {
		const buttonsContainer = container.appendChild($('.hify-chat-multidiff-contributed-buttons'));
		const disposables = new DisposableStore();
	...
	private renderFilesList(container: HTMLElement): IDisposable {
		...
		const listContainer = container.appendChild($('.hify-chat-summary-list'));
