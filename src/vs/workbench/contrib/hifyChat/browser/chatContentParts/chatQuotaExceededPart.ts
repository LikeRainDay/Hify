...
export class ChatQuotaExceededPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		this.domNode = $('.hify-chat-quota-error-widget');
		const icon = dom.append(this.domNode, $('span'));
		icon.classList.add(...ThemeIcon.asClassNameArray(Codicon.warning));

		const messageContainer = dom.append(this.domNode, $('.hify-chat-quota-error-message'));
		...
		const addWaitWarningIfNeeded = () => {
			...
			dom.append(messageContainer, $('.hify-chat-quota-wait-warning', undefined, localize('waitWarning', "Changes may take a few minutes to take effect.")));
		};
		...
		const addRetryButtonIfNeeded = () => {
			...
			button2.element.classList.add('hify-chat-quota-error-secondary-button');
		...
		if (button1Label) {
			...
			button1.element.classList.add('hify-chat-quota-error-button');
			this._register(button1.onDidClick(async () => {
				...
				telemetryService.publicLog2<WorkbenchActionExecutedEvent, WorkbenchActionExecutedClassification>('workbenchActionExecuted', { id: commandId, from: 'hify-chat-response' });
