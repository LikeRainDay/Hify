...
export class ChatThinkingContentPart extends ChatCollapsibleContentPart implements IChatContentPart {
	...
	private defaultTitle = localize('hify.chat.thinking.header', 'Thinking...');
	...
	constructor(
		...
		const extractedTitle = extractTitleFromThinkingContent(initialText)
			?? localize('hify.chat.thinking.header', 'Thinking...');
		...
		const mode = this.configurationService.getValue<string>('hify.chat.agent.thinkingStyle') ?? 'none';
		...
		if (this.perItemCollapsedMode) {
			this.setExpanded(true);
			const header = this.domNode.querySelector('.hify-chat-used-context-label');
			if (header) {
				header.remove();
				this.domNode.classList.add('hify-chat-thinking-no-outer-header');
				this._onDidChangeHeight.fire();
			}
		}

		const node = this.domNode;
		node.classList.add('hify-chat-thinking-box');
		node.tabIndex = 0;
	...
	protected override initContent(): HTMLElement {
		this.wrapper = $('.hify-chat-used-context-list.hify-chat-thinking-collapsible');
		this.wrapper.classList.toggle('hify-chat-thinking-per-item-mode', this.perItemCollapsedMode);
		if (this.perItemCollapsedMode) {
			this.createThinkingItemContainer();
		} else {
			this.textContainer = $('.hify-chat-thinking-item.markdown-content');
	...
	private createThinkingItemContainer(): void {
		const itemWrapper = $('.hify-chat-thinking-item-wrapper');

		const header = $('.hify-chat-thinking-item-header');

		const labelSpan = document.createElement('span');
		labelSpan.classList.add('hify-chat-thinking-item-header-label');
		labelSpan.textContent = this.currentTitle ?? this.defaultTitle;
		header.appendChild(labelSpan);

		const icon = $('.codicon.codicon-chevron-right.hify-chat-thinking-item-caret', { 'aria-hidden': 'true' });
		header.appendChild(icon);

		header.tabIndex = 0;

		const body = $('.hify-chat-thinking-item.markdown-content');
	...
	public finalizeTitleIfDefault(): void {
		if (this.currentTitle === this.defaultTitle) {
			const done = localize('hify.chat.pinned.thinking.header.done', 'Thought for a few seconds...');
			this.setTitle(done);
	...
	protected override setTitle(title: string): void {
		...
		if (this.currentHeaderElement) {
			const labelSpan = this.currentHeaderElement.querySelector('.hify-chat-thinking-item-header-label');
			if (labelSpan) {
