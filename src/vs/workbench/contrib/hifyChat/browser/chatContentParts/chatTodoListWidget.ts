...
export class ChatTodoListWidget extends Disposable {
	...
	private createChatTodoWidget(): HTMLElement {
		const container = dom.$('.hify-chat-todo-list-widget');
		...
		titleElement.textContent = localize('hify.chat.todoList.title', 'Todos');
	...
	private createClearButton(): void {
		this.clearButton = new Button(this.clearButtonContainer, {
			supportIcons: true,
			title: localize('hify.chat.todoList.clearButton', 'Clear all todos'),
			ariaLabel: localize('hify.chat.todoList.clearButton.ariaLabel', 'Clear all todos')
		});
	...
	private renderTodoList(todoList: IChatTodo[]): void {
		...
		todoList.forEach((todo, index) => {
			...
			const ariaLabel = todo.description && todo.description.trim()
				? localize('hify.chat.todoList.itemWithDescription', '{0}, {1}, {2}', todo.title, statusText, todo.description)
				: localize('hify.chat.todoList.item', '{0}, {1}', todo.title, statusText);
	...
	private updateTitleElement(titleElement: HTMLElement, todoList: IChatTodo[]): void {
		...
		if (totalCount === 0) {
			progressText.textContent = localize('hify.chat.todoList.title', 'Todos');
		} else {
			progressText.textContent = localize('hify.chat.todoList.titleWithProgress', 'Todos ({0}/{1})', completedCount, totalCount);
		}
		titleElement.appendChild(progressText);
		const expandButtonLabel = this._isExpanded
			? localize('hify.chat.todoList.collapseButtonWithProgress', 'Collapse {0}', progressText.textContent)
			: localize('hify.chat.todoList.expandButtonWithProgress', 'Expand {0}', progressText.textContent);
	...
	private getStatusText(status: string): string {
		switch (status) {
			case 'completed':
				return localize('hify.chat.todoList.status.completed', 'completed');
			case 'in-progress':
				return localize('hify.chat.todoList.status.inProgress', 'in progress');
			case 'not-started':
			default:
				return localize('hify.chat.todoList.status.notStarted', 'not started');
		}
	}
