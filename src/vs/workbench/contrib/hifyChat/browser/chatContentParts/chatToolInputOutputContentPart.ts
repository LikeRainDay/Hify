...
export class ChatCollapsibleInputOutputContentPart extends Disposable {
	...
	constructor(
		...
		const container = dom.h('.hify-chat-confirmation-widget-container');
		const titleEl = dom.h('.hify-chat-confirmation-widget-title-inner');
		const iconEl = dom.h('.hify-chat-confirmation-widget-title-icon');
		const elements = dom.h('.hify-chat-confirmation-widget');
		...
		btn.element.classList.add('hify-chat-confirmation-widget-title', 'monaco-text-button');
		...
		const message = dom.h('.hify-chat-confirmation-widget-message');
		...
		if (topLevelResources?.length) {
			...
			group.classList.add('hify-chat-collapsible-top-level-resource-group');
			this._register(autorun(r => {
	...
	private createMessageContents() {
		...
		contents.inputTitle.textContent = localize('hify.chat.input', "Input");
		...
		if (!output) {
			...
			contents.outputTitle.textContent = localize('hify.chat.output', "Output");
	...
	private addResourceGroup(parts: IChatCollapsibleIODataPart[], container: HTMLElement) {
		const el = dom.h('.hify-chat-collapsible-io-resource-group', [
			dom.h('.hify-chat-collapsible-io-resource-items@items'),
			dom.h('.hify-chat-collapsible-io-resource-actions@actions'),
		]);
...
class SaveResourcesAction extends Action2 {
	public static readonly ID = 'hify.chat.toolOutput.save';
	constructor() {
		super({
			id: SaveResourcesAction.ID,
			title: localize2('hify.chat.saveResources', "Save As..."),
	...
	async run(accessor: ServicesAccessor, context: IChatToolOutputResourceToolbarContext) {
		...
		const savePart = async (part: IChatCollapsibleIODataPart, isFolder: boolean, uri: URI) => {
			...
			try {
				...
				notificationService.error(localize('hify.chat.saveResources.error', "Failed to save {0}: {1}", basename(part.uri), e));
			}
		};
		...
		const withProgress = async (thenReveal: URI, todo: (() => Promise<void>)[]) => {
			await progressService.withProgress({
				...
				delay: 5_000,
				title: localize('hify.chat.saveResources.progress', "Saving resources..."),
			}, async report => {
			...
			if (workspaceContextService.isInsideWorkspace(thenReveal)) {
				...
				notificationService.info(localize('hify.chat.saveResources.reveal', "Saved resources to {0}", labelService.getUriLabel(thenReveal)));
			}
		};
		...
		if (context.parts.length === 1) {
			...
			const uris = await fileDialog.showOpenDialog({
				title: localize('hify.chat.saveResources.title', "Pick folder to save resources"),
