...
export class ChatToolInvocationPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		this.domNode = dom.$('.hify-chat-tool-invocation-part');
	...
	private get autoApproveMessageContent() {
		...
		switch (reason.type) {
			case ToolConfirmKind.Setting:
				md = localize('hify.chat.autoapprove.setting', 'Auto approved by {0}', markdownCommandLink({ title: '`' + reason.id + '`', id: 'workbench.action.openSettings', arguments: [reason.id] }, false));
				break;
			case ToolConfirmKind.LmServicePerTool:
				md = reason.scope === 'session'
					? localize('hify.chat.autoapprove.lmServicePerTool.session', 'Auto approved for this session')
					: reason.scope === 'workspace'
						? localize('hify.chat.autoapprove.lmServicePerTool.workspace', 'Auto approved for this workspace')
						: localize('hify.chat.autoapprove.lmServicePerTool.profile', 'Auto approved for this profile');
	...
	private createApprovalMessage(): HTMLElement | undefined {
		...
		result.element.classList.add('hify-chat-tool-approval-message');
