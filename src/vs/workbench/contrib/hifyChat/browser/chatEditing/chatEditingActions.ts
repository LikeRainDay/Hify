...
export async function discardAllEditsWithConfirmation(accessor: ServicesAccessor, currentEditingSession: IChatEditingSession): Promise<boolean> {
	...
	if (entries.length > 0) {
		const confirmation = await dialogService.confirm({
			title: localize('hify.chat.editing.discardAll.confirmation.title', "Undo all edits?"),
			message: entries.length === 1
				? localize('hify.chat.editing.discardAll.confirmation.oneFile', "This will undo changes made in {0}. Do you want to proceed?", basename(entries[0].modifiedURI))
				: localize('hify.chat.editing.discardAll.confirmation.manyFiles', "This will undo changes made in {0} files. Do you want to proceed?", entries.length),
			primaryButton: localize('hify.chat.editing.discardAll.confirmation.primaryButton', "Yes"),
			type: 'info'
		});
...
async function restoreSnapshotWithConfirmation(accessor: ServicesAccessor, item: ChatTreeItem): Promise<void> {
	...
	if (requestId) {
		...
		const shouldPrompt = entriesModifiedInRequestsToRemove.length > 0 && configurationService.getValue('hify.chat.editing.confirmEditRequestRemoval') === true;

		let message: string;
		if (editsToUndo === 1) {
			if (entriesModifiedInRequestsToRemove.length === 1) {
				message = localize('hify.chat.removeLast.confirmation.message2', "This will remove your last request and undo the edits made to {0}. Do you want to proceed?", basename(entriesModifiedInRequestsToRemove[0].modifiedURI));
			} else {
				message = localize('hify.chat.removeLast.confirmation.multipleEdits.message', "This will remove your last request and undo edits made to {0} files in your working set. Do you want to proceed?", entriesModifiedInRequestsToRemove.length);
			}
		} else {
			if (entriesModifiedInRequestsToRemove.length === 1) {
				message = localize('hify.chat.remove.confirmation.message2', "This will remove all subsequent requests and undo edits made to {0}. Do you want to proceed?", basename(entriesModifiedInRequestsToRemove[0].modifiedURI));
			} else {
				message = localize('hify.chat.remove.confirmation.multipleEdits.message', "This will remove all subsequent requests and undo edits made to {0} files in your working set. Do you want to proceed?", entriesModifiedInRequestsToRemove.length);
			}
		}
		...
		const confirmation = shouldPrompt
			? await dialogService.confirm({
				title: editsToUndo === 1
					? localize('hify.chat.removeLast.confirmation.title', "Do you want to undo your last edit?")
					: localize('hify.chat.remove.confirmation.title', "Do you want to undo {0} edits?", editsToUndo),
				message: message,
				primaryButton: localize('hify.chat.remove.confirmation.primaryButton', "Yes"),
				checkbox: { label: localize('hify.chat.remove.confirmation.checkbox', "Don't ask again"), checked: false },
				type: 'info'
			})
			: { confirmed: true };
		...
		if (confirmation.checkboxChecked) {
			await configurationService.updateValue('hify.chat.editing.confirmEditRequestRemoval', false);
		}
...
registerAction2(class RemoveAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('hify.chat.undoEdits.label', "Undo Requests"),
			f1: false,
			category: CHAT_CATEGORY,
			icon: Codicon.discard,
			keybinding: {
	...
	async run(accessor: ServicesAccessor, ...args: any[]) {
		...
		if (isRequestVM(item) && configurationService.getValue('hify.chat.undoRequests.restoreInput')) {
			widget?.focusInput();
...
registerAction2(class RestoreCheckpointAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('hify.chat.restoreCheckpoint.label', "Restore Checkpoint"),
			tooltip: localize2('hify.chat.restoreCheckpoint.tooltip', "Restores workspace and chat to this point"),
			f1: false,
			category: CHAT_CATEGORY,
			keybinding: {
				primary: KeyCode.Delete,
...
registerAction2(class RestoreLastCheckpoint extends Action2 {
	constructor() {
		super({
			...
			title: localize2('hify.chat.restoreLastCheckpoint.label', "Restore to Last Checkpoint"),
			f1: false,
			category: CHAT_CATEGORY,
			icon: Codicon.discard,
			menu: [
...
registerAction2(class EditAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('hify.chat.editRequests.label', "Edit Request"),
			f1: false,
			category: CHAT_CATEGORY,
			icon: Codicon.edit,
			keybinding: {
...
registerAction2(class OpenWorkingSetHistoryAction extends Action2 {
	...
	static readonly id = 'hify.chat.openFileUpdatedBySnapshot';
	constructor() {
		super({
			id: OpenWorkingSetHistoryAction.id,
			title: localize('hify.chat.openFileUpdatedBySnapshot.label', "Open File"),
			menu: [{
...
registerAction2(class OpenWorkingSetHistoryAction extends Action2 {
	...
	static readonly id = 'hify.chat.openFileSnapshot';
	constructor() {
		super({
			id: OpenWorkingSetHistoryAction.id,
			title: localize('hify.chat.openSnapshot.label', "Open File Snapshot"),
			menu: [{
				id: MenuId.ChatEditingCodeBlockContext,
				group: 'navigation',
				order: 1,
