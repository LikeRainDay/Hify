...
class SimpleBrowserOverlayWidget {
	...
	constructor(
		...
		this._showStore.add(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('hify.chat.sendElementsToChat.enabled')) {
				if (this.configurationService.getValue('hify.chat.sendElementsToChat.enabled')) {
		...
		this.imagesFolder = joinPath(this.environmentService.workspaceStorageHome, 'vscode-hify-chat-images');
		...
		const configure = this._showStore.add(new Button(this._domNode, { supportIcons: true, title: localize('hify.chat.configureElements', "Configure Attachments Sent") }));
		configure.icon = Codicon.gear;

		const collapseOverlay = this._showStore.add(new Button(this._domNode, { supportIcons: true, title: localize('hify.chat.hideOverlay', "Collapse Overlay") }));
		collapseOverlay.icon = Codicon.chevronRight;

		const nextSelection = this._showStore.add(new Button(this._domNode, { supportIcons: true, title: localize('hify.chat.nextSelection', "Select Again") }));
		nextSelection.icon = Codicon.close;
		nextSelection.element.classList.add('hidden');

		// shown if the overlay is collapsed
		const expandOverlay = this._showStore.add(new Button(this._domNode, { supportIcons: true, title: localize('hify.chat.expandOverlay', "Expand Overlay") }));
	...
	async addElementToChat(cts: CancellationTokenSource) {
		...
		if (this.configurationService.getValue('hify.chat.sendElementsToChat.attachCSS')) {
		...
		if (this.configurationService.getValue('hify.chat.sendElementsToChat.attachImages')) {
...
class SimpleBrowserOverlayController {
	...
	constructor(
		...
		this._domNode.classList.add('hify-chat-simple-browser-overlay');
...
export class SimpleBrowserOverlay implements IWorkbenchContribution {
	...
	static readonly ID = 'hify.chat.simpleBrowser.overlay';
