...
const ChatEditorIcon = registerIcon('hify-chat-editor-label-icon', Codicon.chatSparkle, nls.localize('chatEditorLabelIcon', 'Icon of the chat editor label.'));
...
export class ChatEditorInput extends EditorInput implements IEditorCloseHandler {
	...
	async confirm(editors: ReadonlyArray<IEditorIdentifier>): Promise<ConfirmResult> {
		...
		const messageOverride = nls.localize('hify.chat.startEditing.confirmation.pending.message.default', "Closing the chat editor will end your current edit session.");
...
export namespace ChatEditorUri {
	...
	export function generate(handle: number): URI {
		return URI.from({ scheme, path: `hify-chat-${handle}` });
	}
	...
	export function parse(resource: URI): number | undefined {
		...
		const match = resource.path.match(/hify-chat-(\d+)/);
...
export async function showClearEditingSessionConfirmation(editingSession: IChatEditingSession, dialogService: IDialogService, options?: IClearEditingSessionConfirmationOptions): Promise<boolean> {
	const defaultPhrase = nls.localize('hify.chat.startEditing.confirmation.pending.message.default1', "Starting a new chat will end your current edit session.");
	const defaultTitle = nls.localize('hify.chat.startEditing.confirmation.title', "Start new chat?");
	...
	const { result } = await dialogService.prompt({
		title,
		message: phrase + ' ' + nls.localize('hify.chat.startEditing.confirmation.pending.message.2', "Do you want to keep pending edits to {0} files?", undecidedEdits.length),
		type: 'info',
		cancelButton: true,
		buttons: [
			{
				label: nls.localize('hify.chat.startEditing.confirmation.acceptEdits', "Keep & Continue"),
				run: async () => {
			...
			{
				label: nls.localize('hify.chat.startEditing.confirmation.discardEdits', "Undo & Continue"),
				run: async () => {
