...
const GlobalLastChatModeKey = 'hify.chat.lastChatMode';
...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	public getAttachedAndImplicitContext(sessionId: string): ChatRequestVariableSet {
		...
		if ((this.implicitContext?.enabled && this.implicitContext?.value) || (isLocation(this.implicitContext?.value) && this.configurationService.getValue<boolean>('hify.chat.implicitContext.suggestedContext'))) {
	...
	constructor(
		...
		this._register(this.onDidChangeCurrentChatMode(() => {
			...
			if (this.implicitContext && this.configurationService.getValue<boolean>('hify.chat.implicitContext.suggestedContext')) {
	...
	initForNewChatModel(state: IChatViewState, modelIsEmpty: boolean): void {
		...
		if (modelIsEmpty) {
			...
			if (!hasSetDefaultMode) {
				const defaultModeKey = this.entitlementService.entitlement === ChatEntitlement.Free ? 'hify.chat.defaultModeFree' : 'hify.chat.defaultMode';
				const defaultLanguageModelKey = this.entitlementService.entitlement === ChatEntitlement.Free ? 'hify.chat.defaultLanguageModelFree' : 'hify.chat.defaultLanguageModel';
				Promise.all([
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		if (this.options.renderStyle === 'compact') {
			elements = dom.h('.interactive-input-part', [
				dom.h('.interactive-input-and-edit-session', [
					dom.h('.hify-chat-editing-session@chatEditingSessionWidgetContainer'),
					dom.h('.interactive-input-and-side-toolbar@inputAndSideToolbar', [
						dom.h('.hify-chat-input-container@inputContainer', [
							dom.h('.hify-chat-editor-container@editorContainer'),
							dom.h('.hify-chat-input-toolbars@inputToolbars'),
						]),
					]),
					dom.h('.hify-chat-attachments-container@attachmentsContainer', [
						dom.h('.hify-chat-attachment-toolbar@attachmentToolbar'),
						dom.h('.hify-chat-attached-context@attachedContextContainer'),
						dom.h('.hify-chat-related-files@relatedFilesContainer'),
					]),
		...
		this.chatInputOverlay = dom.$('.hify-chat-input-overlay');
		...
		this.inputActionsToolbar.getElement().classList.add('hify-chat-input-toolbar');
			...
			actionViewItemProvider: (action, options) => {
				if (this.location === ChatAgentLocation.Chat || this.location === ChatAgentLocation.EditorInline) {
					if ((action.id === ChatSubmitAction.ID || action.id === CancelAction.ID || action.id === ChatEditingSessionSubmitAction.ID) && action instanceof MenuItemAction) {
						const dropdownAction = this.instantiationService.createInstance(MenuItemAction, { id: 'hify.chat.moreExecuteActions', title: localize('notebook.moreExecuteActionsLabel', "More..."), icon: Codicon.chevronDown }, undefined, undefined, undefined, undefined);
		...
		this.executeToolbar.getElement().classList.add('hify-chat-execute-toolbar');
		...
		if (this.options.menus.inputSideToolbar) {
			...
			toolbarSide.getElement().classList.add('hify-chat-side-toolbar');
	...
	public renderAttachedContext() {
		...
		const isSuggestedEnabled = this.configurationService.getValue<boolean>('hify.chat.implicitContext.suggestedContext');
	...
	private handleAttachmentDeletion(e: KeyboardEvent | unknown, index: number, attachment: IChatRequestVariableEntry) {
		...
		if (this.configurationService.getValue<boolean>('hify.chat.implicitContext.enableImplicitContext')) {
	...
	private handleAttachmentNavigation(e: StandardKeyboardEvent): void {
		...
		const attachments = Array.from(this.attachedContextContainer.querySelectorAll('.hify-chat-attached-context-attachment'));
		if (!attachments.length) {
			return;
		}
	...
	async renderChatEditingSessionState(chatEditingSession: IChatEditingSession | null) {
		...
		const innerContainer = this.chatEditingSessionWidgetContainer.querySelector('.hify-chat-editing-session-container.show-file-icons') as HTMLElement ?? dom.append(this.chatEditingSessionWidgetContainer, $('.hify-chat-editing-session-container.show-file-icons'));
		...
		const overviewRegion = innerContainer.querySelector('.hify-chat-editing-session-overview') as HTMLElement ?? dom.append(innerContainer, $('.hify-chat-editing-session-overview'));
		...
		const actionsContainer = overviewRegion.querySelector('.hify-chat-editing-session-actions') as HTMLElement ?? dom.append(overviewRegion, $('.hify-chat-editing-session-actions'));
		...
		// Working set
		const workingSetContainer = innerContainer.querySelector('.hify-chat-editing-session-list') as HTMLElement ?? dom.append(innerContainer, $('.hify-chat-editing-session-list'));
	...
	async renderChatRelatedFiles() {
		...
		const shouldRender = this.configurationService.getValue('hify.chat.renderRelatedFiles');
...
class AddFilesButton extends ActionViewItem {
	...
	override render(container: HTMLElement): void {
		container.classList.add('hify-chat-attachment-button');
