...
export function isChatSession(editor?: EditorInput): boolean {
	...
	if (editor.resource?.scheme !== 'vscode-hify-chat-editor' && editor.resource?.scheme !== 'vscode-hify-chat-session') {
		return false;
	}
...
export function getChatSessionType(editor: ChatEditorInput): string {
	...
	// Default to 'local' for vscode-hify-chat-editor scheme or when type cannot be determined
	return 'local';
}
