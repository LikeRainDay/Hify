...
export class SessionsRenderer extends Disposable implements ITreeRenderer<IChatSessionItem, FuzzyScore, ISessionTemplateData> {
	...
	private applyIconColorStyle(iconId: string, colorId: string): void {
		...
		if (color) {
			...
			const css = `.monaco-workbench .hify-chat-session-item .monaco-icon-label.codicon-${iconId}::before { color: ${color} !important; }`;
			const activeWindow = getActiveWindow();

			const styleId = `hify-chat-sessions-icon-${styleKey}`;
	...
	renderTemplate(container: HTMLElement): ISessionTemplateData {
		const element = append(container, $('.hify-chat-session-item'));
