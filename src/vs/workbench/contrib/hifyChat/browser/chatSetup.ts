...
class ChatSetup {
	...
	private async showDialog(options?: { forceSignInDialog?: boolean }): Promise<ChatSetupStrategy> {
		...
		const dialogVariant = this.configurationService.getValue<'default' | 'apple' | unknown>('hify.chat.setup.signInDialogVariant');
		...
		const dialog = disposables.add(new Dialog(
			...
			createWorkbenchDialogOptions({
				type: 'none',
				extraClasses: ['hify-chat-setup-dialog'],
	...
	private createDialogFooter(disposables: DisposableStore): HTMLElement {
		const element = $('.hify-chat-setup-dialog-footer');
		...
		class UpgradePlanAction extends Action2 {
			constructor() {
				super({
					...
					category: localize2('hify.chat.category', 'Chat'),
					f1: true,
...
export class ChatTeardownContribution extends Disposable implements IWorkbenchContribution {
	...
	static readonly CHAT_DISABLED_CONFIGURATION_KEY = 'hify.chat.disableAIFeatures';
