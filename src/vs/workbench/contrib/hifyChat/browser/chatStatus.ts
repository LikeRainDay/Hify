	...
	private update(): void {
		...
		if (!sentiment.hidden) {
			...
			if (this.entry) {
				...
				this.entry = this.statusbarService.addEntry(props, 'hify.chat.statusBarEntry', StatusbarAlignment.RIGHT, { location: { id: 'status.editor.mode', priority: 100.1 }, alignment: StatusbarAlignment.RIGHT });
			}
		} else {
			this.entry?.dispose();
...
class ChatStatusDashboard extends Disposable {
	...
	private readonly element = $('div.hify-chat-status-bar-entry-tooltip');
	...
	private runCommandAndClose(commandOrFn: string | Function): void {
		if (typeof commandOrFn === 'function') {
			...
			this.telemetryService.publicLog2<WorkbenchActionExecutedEvent, WorkbenchActionExecutedClassification>('workbenchActionExecuted', { id: commandOrFn, from: 'hify-chat-status' });
