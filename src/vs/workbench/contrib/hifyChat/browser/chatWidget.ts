...
class ChatHistoryListRenderer implements IListRenderer<IChatHistoryListItem, IChatHistoryTemplate> {
	...
	renderTemplate(container: HTMLElement): IChatHistoryTemplate {
		...
		container.classList.add('hify-chat-welcome-history-item');
		const title = dom.append(container, $('.hify-chat-welcome-history-title'));
		const date = dom.append(container, $('.hify-chat-welcome-history-date'));
	...
	renderElement(element: IChatHistoryListItem, index: number, templateData: IChatHistoryTemplate): void {
		...
		const titleHoverEl = dom.$('div.hify-chat-history-item-hover');
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	constructor(
		...
		this._register(this.configurationService.onDidChangeConfiguration((e) => {
			if (e.affectsConfiguration('hify.chat.renderRelatedFiles')) {
	...
	get input(): ChatInputPart {
		return this.viewModel?.editing && this.configurationService.getValue<string>('hify.chat.editRequests') !== 'input' ? this.inlineInputPart : this.inputPart;
	}
	...
	render(parent: HTMLElement): void {
		...
		this.welcomeMessageContainer = dom.append(this.container, $('.hify-chat-welcome-view-container', { style: 'display: none' }));
		...
		scrollDownButton.element.classList.add('hify-chat-scroll-down');
		...
		this._register(autorun(reader => {
			...
			this.container.style.setProperty('--vscode-hify-chat-font-family', fontFamily);
	...
	private renderWelcomeViewContentIfNeeded() {
		...
		if (!numItems) {
			const expEmptyState = this.configurationService.getValue<boolean>('hify.chat.emptyChatState.enabled');
	...
	private async renderWelcomeHistorySection(): Promise<void> {
		try {
			const historyRoot = dom.append(this.welcomeMessageContainer, $('.hify-chat-welcome-history-root'));
			const container = dom.append(historyRoot, $('.hify-chat-welcome-history'));
			const header = dom.append(container, $('.hify-chat-welcome-history-header'));
			const headerTitle = dom.append(header, $('.hify-chat-welcome-history-header-title'));
			headerTitle.textContent = localize('hify.chat.history.title', 'History');
			const headerActions = dom.append(header, $('.hify-chat-welcome-history-header-actions'));
			...
			const showAllButton = dom.append(headerActions, $('.hify-chat-welcome-history-show-all'));
			showAllButton.classList.add('codicon', `codicon-${Codicon.history.id}`, 'hify-chat-welcome-history-show-all');
			showAllButton.tabIndex = 0;
			showAllButton.setAttribute('role', 'button');
			const showAllHover = localize('hify.chat.history.showAllHover', 'Show history...');
			showAllButton.setAttribute('aria-label', showAllHover);
			const showAllHoverText = dom.$('div.hify-chat-history-button-hover');
			...
			const welcomeHistoryContainer = dom.append(container, $('.hify-chat-welcome-history-list'));

			this.welcomeMessageContainer.classList.toggle('has-hify-chat-history', filtered.length > 0);
				...
				const list = this.instantiationService.createInstance(
					...
					[renderer],
					{
						...
						accessibilityProvider: {
							...
							getWidgetAriaLabel: () => localize('hify.chat.history.list', 'Chat History')
						}
					}
				);
	...
	private createList(listContainer: HTMLElement, options: IChatListItemRendererOptions): void {
		...
		overflowWidgetsContainer.classList.add('hify-chat-overflow-widget-container', 'monaco-editor');
		...
		this._register(this.renderer.onDidRerender(item => {
			if (isRequestVM(item.currentElement) && this.configurationService.getValue<string>('hify.chat.editRequests') !== 'input') {
	...
	private clickedRequest(item: IChatListItemTemplate) {
		...
		if (isRequestVM(currentElement) && !this.viewModel?.editing) {
			...
			const isInput = this.configurationService.getValue<string>('hify.chat.editRequests') === 'input';
			this.inputPart?.setEditing(!!this.viewModel?.editing && isInput);

			if (!isInput) {
				const rowContainer = item.rowContainer;
				this.inputContainer = dom.$('.hify-chat-edit-input-container');
			...
			this._register(this.inputPart.onDidClickOverlay(() => {
				if (this.viewModel?.editing && this.configurationService.getValue<string>('hify.chat.editRequests') !== 'input') {
					this.finishedEditing();
				}
			}));
		...
		this.telemetryService.publicLog2<StartRequestEvent, StartRequestEventClassification>('hify.chat.startEditingRequests', {
			editRequestType: this.configurationService.getValue<string>('hify.chat.editRequests'),
		});
	}
	...
	finishedEditing(completedEdit?: boolean): void {
		...
		if (!isInput) {
			...
			this.inputContainer = dom.$('.empty-hify-chat-state');
		...
		this.telemetryService.publicLog2<CancelRequestEditEvent, CancelRequestEventEditClassification>('hify.chat.editRequestsFinished', {
			editRequestType: this.configurationService.getValue<string>('hify.chat.editRequests'),
	...
	private onDidStyleChange(): void {
		...
		this.container.style.setProperty('--vscode-hify-chat-list-background', this.themeService.getColorTheme().getColor(this.styles.listBackground)?.toString() ?? '');
	}
	...
	setModel(model: IChatModel, viewState: IChatViewState): void {
		...
		if (this._lockedToCodingAgent) {
			const placeholder = localize('hify.chat.input.placeholder.lockedToAgent', "Chat with {0}", this._lockedToCodingAgent);
		...
		const renderImmediately = this.configurationService.getValue<boolean>('hify.chat.experimental.renderMarkdownImmediately');
	...
	layout(height: number, width: number): void {
		...
		if (this.viewOptions.renderStyle === 'compact' || this.viewOptions.renderStyle === 'minimal') {
			this.listContainer.style.removeProperty('--hify-chat-current-response-min-height');
		} else {
			this.listContainer.style.setProperty('--hify-chat-current-response-min-height', contentHeight * .75 + 'px');
		}
