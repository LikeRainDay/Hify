...
export class CodeBlockPart extends Disposable {
	...
	constructor(
		...
		this.editor = this.createEditor(scopedInstantiationService, editorElement, {
			...
			ariaLabel: localize('hify.chat.codeBlockHelp', 'Code block'),
		...
		this._register(this.vulnsButton.onDidClick(() => {
			...
			this.element.classList.toggle('hify-chat-vulnerabilities-collapsed', !element.vulnerabilitiesListExpanded);
	...
	async render(data: ICodeBlockData, width: number) {
		...
		if (!this.editor.getOption(EditorOption.ariaLabel)) {
			...
			this.editor.updateOptions({
				ariaLabel: localize('hify.chat.codeBlockLabel', "Code block {0}", data.codeBlockIndex + 1),
			});
		}
		this.layout(width);
		this.toolbar.setAriaLabel(localize('hify.chat.codeBlockToolbarLabel', "Code block {0}", data.codeBlockIndex + 1));
		...
		if (data.vulns?.length && isResponseVM(data.element)) {
			...
			this.element.classList.toggle('hify-chat-vulnerabilities-collapsed', !data.element.vulnerabilitiesListExpanded);
			dom.append(this.vulnsListElement, ...data.vulns.map(v => $('li', undefined, $('span.hify-chat-vuln-title', undefined, v.title), ' ' + v.description)));
			this.vulnsButton.label = this.getVulnerabilitiesLabel();
		} else {
...
export class CodeCompareBlockPart extends Disposable {
	...
	private _configureForScreenReader(): void {
		...
		if (this.accessibilityService.isScreenReaderOptimized()) {
			toolbarElt.style.display = 'block';
			toolbarElt.ariaLabel = localize('hify.chat.codeBlock.toolbar', 'Code block toolbar');
		} else {
			toolbarElt.style.display = '';
		}
	}
	...
	async render(data: ICodeCompareBlockData, width: number, token: CancellationToken) {
		...
		this.layout(width);
		this.diffEditor.updateOptions({ ariaLabel: localize('hify.chat.compareCodeBlockLabel', "Code Edits") });
	...
	private async updateEditor(data: ICodeCompareBlockData, token: CancellationToken): Promise<void> {
		...
		if (isEditApplied) {
			...
			if (data.edit.state.applied === 1) {
				template = localize('hify.chat.edits.1', "Applied 1 change in [[``{0}``]]", uriLabel);
			} else if (data.edit.state.applied < 0) {
				template = localize('hify.chat.edits.rejected', "Edits in [[``{0}``]] have been rejected", uriLabel);
			} else {
				template = localize('hify.chat.edits.N', "Applied {0} changes in [[``{1}``]]", data.edit.state.applied, uriLabel);
			}
