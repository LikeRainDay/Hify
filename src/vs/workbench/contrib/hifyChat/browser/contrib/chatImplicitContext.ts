...
export class ChatImplicitContextContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'hify.chat.implicitContext';
	...
	constructor(
		...
		this._implicitContextEnablement = this.configurationService.getValue<{ [mode: string]: string }>('hify.chat.implicitContext.enabled');
		...
		this._register(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('hify.chat.implicitContext.enabled')) {
				this._implicitContextEnablement = this.configurationService.getValue<{ [mode: string]: string }>('hify.chat.implicitContext.enabled');
				this.updateImplicitContext();
			}
		}));
	...
	private async updateImplicitContext(updateWidget?: IChatWidget): Promise<void> {
		...
		if (model) {
			...
			if (selection && !selection.isEmpty()) {
				...
				if (this.configurationService.getValue('hify.chat.implicitContext.suggestedContext')) {
					newValue = model.uri;
				} else {
