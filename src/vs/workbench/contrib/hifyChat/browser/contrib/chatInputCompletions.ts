		...
		this._register(this.languageFeaturesService.completionProvider.register({ scheme: Schemas.vscodeChatInput, hasAccessToAllModels: true }, {
			...
			provideCompletionItems: async (model: ITextModel, position: Position, _context: CompletionContext, token: CancellationToken) => {
				...
				const item: CompletionItem = {
					...
					range,
					kind: CompletionItemKind.Text, // The icons are disabled here anyway
					command: { id: 'workbench.extensions.search', title: '', arguments: ['@tag:hify-chat-participant'] },
		...
		model.changeDecorations(accessor => {
			const id = accessor.addDecoration(lastPosition, {
				...
				after: {
					...
					inlineClassName: ThemeIcon.asClassName(ThemeIcon.modify(Codicon.loading, 'spin')) + ' hify-chat-prompt-spinner',
				}
			});
