...
export class UserToolSetsContributions extends Disposable implements IWorkbenchContribution {
	...
	static readonly ID = 'hify.chat.userToolSets';
...
export class ConfigureToolSets extends Action2 {
	...
	static readonly ID = 'hify.chat.configureToolSets';

	constructor() {
		super({
			id: ConfigureToolSets.ID,
			title: localize2('hify.chat.configureToolSets', 'Configure Tool Sets...'),
			shortTitle: localize('hify.chat.configureToolSets.short', "Tool Sets"),
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		picks.push({
			label: localize('hify.chat.configureToolSets.add', 'Create new tool sets file...'),
			alwaysShow: true,
		...
		const pick = await quickInputService.pick(picks, {
			canPickMany: false,
			placeHolder: localize('hify.chat.configureToolSets.placeholder', 'Select a tool set to configure'),
		});
