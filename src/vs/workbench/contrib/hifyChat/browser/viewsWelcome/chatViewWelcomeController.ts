...
export class ChatViewWelcomeController extends Disposable {
	...
	constructor(
		...
		this.element = dom.append(this.container, dom.$('.hify-chat-view-welcome'));
	...
	private update(force?: boolean): void {
		...
		if (!enabled) {
			this.container.classList.toggle('hify-chat-view-welcome-visible', false);
	...
	private render(descriptors: ReadonlyArray<IChatViewsWelcomeDescriptor>): void {
		...
		if (enabledDescriptor) {
			...
			this.container.classList.toggle('hify-chat-view-welcome-visible', true);
		} else {
			this.container.classList.toggle('hify-chat-view-welcome-visible', false);
		}
	}
...
export class ChatViewWelcomePart extends Disposable {
	...
	constructor(
		...
		this.element = dom.$('.hify-chat-welcome-view');

		try {
			const renderer = this.instantiationService.createInstance(MarkdownRenderer, {});

			// Icon
			const icon = dom.append(this.element, $('.hify-chat-welcome-view-icon'));
			if (content.icon) {
				icon.appendChild(renderIcon(content.icon));
			}

			// Title
			const title = dom.append(this.element, $('.hify-chat-welcome-view-title'));
			...
			if (typeof content.message !== 'function' && options?.isWidgetAgentWelcomeViewContent && !expEmptyState) {
				const container = dom.append(this.element, $('.hify-chat-welcome-view-indicator-container'));
				dom.append(container, $('.hify-chat-welcome-view-subtitle', undefined, localize('agentModeSubtitle', "Agent Mode")));
			}

			// Message
			const message = dom.append(this.element, content.isExperimental ? $('.hify-chat-welcome-experimental-view-message') : $('.hify-chat-welcome-view-message'));
			...
			if (content.isExperimental && content.inputPart) {
				content.inputPart.querySelector('.hify-chat-attachments-container')?.remove();
				dom.append(this.element, content.inputPart);
			}
			if (!content.isExperimental) {
				// Additional message
				if (typeof content.additionalMessage === 'string') {
					const disclaimers = dom.append(this.element, $('.hify-chat-welcome-view-disclaimer'));
			...
			if (content.suggestedPrompts && content.suggestedPrompts.length) {
				const suggestedPromptsContainer = dom.append(this.element, $('.hify-chat-welcome-view-suggested-prompts'));
				for (const prompt of content.suggestedPrompts) {
					const promptElement = dom.append(suggestedPromptsContainer, $('.hify-chat-welcome-view-suggested-prompt'));
					...
					if (prompt.icon) {
						const iconElement = dom.append(promptElement, $('.hify-chat-welcome-view-suggested-prompt-icon'));
						iconElement.appendChild(renderIcon(prompt.icon));
					}
					const labelElement = dom.append(promptElement, $('.hify-chat-welcome-view-suggested-prompt-label'));
					...
					const executePrompt = () => {
						...
						this.telemetryService.publicLog2<SuggestedPromptClickEvent, SuggestedPromptClickData>('hify.chat.clickedSuggestedPrompt', {
			...
			// Tips
			if (content.tips) {
				const tips = dom.append(this.element, $('.hify-chat-welcome-view-tips'));
			...
			if (content.isExperimental && typeof content.additionalMessage === 'string') {
				const additionalMsg = $('.hify-chat-welcome-view-experimental-additional-message');
