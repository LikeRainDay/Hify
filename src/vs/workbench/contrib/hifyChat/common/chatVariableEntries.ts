...
export namespace IDiagnosticVariableEntryFilterData {
	...
	export function label(data: IDiagnosticVariableEntryFilterData) {
		...
		let labelStr = localize('hify.chat.attachment.problems.all', "All Problems");
		if (data.filterUri) {
			labelStr = localize('hify.chat.attachment.problems.inFile', "Problems in {0}", basename(data.filterUri));
		}
