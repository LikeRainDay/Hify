...
suite('LanguageModelToolsService', () => {
	...
	test('accessibility signal for tool confirmation', async () => {
		...
		testConfigService.setUserConfiguration('hify.chat.tools.global.autoApprove', false);
	...
	test('accessibility signal respects autoApprove configuration', async () => {
		...
		const testConfigService = new TestConfigurationService();
		testConfigService.setUserConfiguration('hify.chat.tools.global.autoApprove', true);
		testConfigService.setUserConfiguration('accessibility.signals.chatUserActionRequired', { sound: 'auto', announcement: 'auto' });
	...
	test('shouldAutoConfirm with basic configuration', async () => {
		...
		const testConfigService = new TestConfigurationService();
		testConfigService.setUserConfiguration('hify.chat.tools.global.autoApprove', true); // Global enabled
	...
	test('shouldAutoConfirm with per-tool configuration object', async () => {
		...
		const testConfigService = new TestConfigurationService();
		testConfigService.setUserConfiguration('hify.chat.tools.global.autoApprove', {
			'approvedTool': true,
			'deniedTool': false
		});
	...
	test('accessibility signal with different settings combinations', async () => {
		...
		testConfigService1.setUserConfiguration('hify.chat.tools.global.autoApprove', false);
		...
		testConfigService2.setUserConfiguration('hify.chat.tools.global.autoApprove', false);
		...
		testConfigService3.setUserConfiguration('hify.chat.tools.global.autoApprove', false);
	...
	test('configuration changes trigger tool updates', async () => {
		...
		configurationService.setUserConfiguration('hify.chat.extensionTools.enabled', false);
		// Fire the configuration change event manually
		configurationService.onDidChangeConfigurationEmitter.fire({ affectsConfiguration: () => true, affectedKeys: new Set(['hify.chat.extensionTools.enabled']) } as any as IConfigurationChangeEvent);
	...
	test('shouldAutoConfirm with workspace-specific tool configuration', async () => {
		...
		testConfigService.setUserConfiguration('hify.chat.tools.global.autoApprove', { 'workspaceTool': true });
